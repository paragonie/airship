...
class Cabins extends LoggedInUsersOnly
{
    ...
    {
        if (!\is_dir(ROOT . '/Cabin/Bridge/View/cabin_links/')) {
            \mkdir(ROOT . '/Cabin/Bridge/View/cabin_links/', 0775);
        }
        if (!\is_link(ROOT . '/Cabin/Bridge/View/cabin_links/' . $cabin)) {
            if (\file_exists(ROOT . '/Cabin/Bridge/View/cabin_links/' . $cabin)) {
                \unlink(ROOT . '/Cabin/Bridge/View/cabin_links/' . $cabin);
            }
            return \symlink(
                ROOT . '/Cabin/' . $cabin . '/config/editor_templates',
                ROOT . '/Cabin/Bridge/View/cabin_links/' . $cabin
            );
        }
        return true;
    }
